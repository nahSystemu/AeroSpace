...
/// becomes nil, etc.) which tricks HyprSpace into thinking that all windows were closed.
/// That's why every time a window dies HyprSpace caches the "entire world" (unless window is already presented in the cache)
/// so that once the screen is unlocked, HyprSpace could restore windows to where they were
