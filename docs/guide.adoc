...
:url-search-for-config: https://github.com/search?q=path%3A*hyprspace.toml&type=code
...
HyprSpace provides bash, fish and zsh completions.

You can also install specific previous https://github.com/nahSystemu/homebrew-tap/tree/main/Casks[pinned homebrew Casks versions]:
[source,bash]
----
brew install --cask nahSystemu/tap/hyprspace@0.12.0
----

[#manual-installation]
=== Manual installation

. Download the latest available zip from https://github.com/nahSystemu/HyprSpace/releases[releases page]
. Unpack zip
. Put unpacked `HyprSpace-v$VERSION/HyprSpace.app` to `/Applications`
. Put unpacked `HyprSpace-v$VERSION/bin/hyprspace` anywhere to `$PATH`
(The step is optional. It is only needed if you want to be able to interact with HyprSpace from CLI)

If you see this message

----
"HyprSpace.app" can't be opened because Apple cannot check it for malicious software.
----
...
. navigate in Finder to `/Applications/HyprSpace.app`
. Right mouse click
...
HyprSpace tries to find the custom config in two locations:

. `~/.hyprspace.toml`
. `+${XDG_CONFIG_HOME}/hyprspace/hyprspace.toml+`
...
HyprSpace uses TOML format for the config.
...
If no custom config is found, HyprSpace will load the default config.
...
The initial binding mode that HyprSpace starts out with is "main".
...
Commands is the thing that you use to manipulate HyprSpace and query its state.

There are two ways on how you can use commands:

. Bind keys to run HyprSpace commands. Example:
+
[source,toml]
----
[mode.main.binding]
...
* For `dvorak` and `colemak` users, HyprSpace offers preconfigured presets.
+
[source,toml]
----
[key-mapping]
...
* `exec.inherit-env-vars = true` configures whether inherit environment variables of `HyprSpace.app` or not. (The default is `true`)
...
That's why unless you override `exec` section in your config, HyprSpace fallbacks to the following `exec` configuration:
+
[source,toml]
----
[exec]
...
HyprSpace stores all windows and containers in a tree.
HyprSpace tree tiling model is https://i3wm.org/docs/userguide.html#tree[inspired by i3].

*Definition.* Each non-leaf node is called a "Container"

WARNING: i3 has a different terminology.
"container" in i3 is the same as "node" in HyprSpace.
...
. <<layouts,Layout>> (Possible values: `tiles`, `accordion`, `hyprland`)
...
In total, HyprSpace provides 5 possible layouts:
...
- `hyprland` Hyprland-inspired dwindle layout that alternates split orientation on each depth
...
The `hyprland` layout alternates the split orientation while traversing the container’s children, producing a dwindle spiral. The initial split orientation always comes from the container orientation (`horizontal` or `vertical`). Each split assigns the leading region approximately 62% of the space and hands the remaining area to the rest of the stack, so adding new windows creates the characteristic shrinking spiral. Mouse-based resizing is disabled for `hyprland` containers at the moment; use layout or move commands to reorganize windows.

[#normalization]
=== Normalization

By default, HyprSpace does two types of tree normalizations:
...
Since Spaces are so hard to deal with, HyprSpace reimplements Spaces and calls them "Workspaces".
...
When you quit the HyprSpace or when the HyprSpace detects that it's about to crash, HyprSpace will place all windows back to the visible area of the screen.

HyprSpace shows the name of currently active workspace in its tray icon (top right corner), to give users a visual feedback on what workspace is currently active.

The intended workflow of using HyprSpace workspaces is to only have one macOS Space (or as many monitors you have, if `Displays have separate Spaces` is enabled) and don’t interact with macOS Spaces anymore.
...
That means, that if HyprSpace crashes badly you will still be able to manually "unhide" the windows by dragging these few pixels to the center of the screen.
...
Since HyprSpace needs a free space to hide windows in,
...
For some reason, mission control doesn't like that HyprSpace puts a lot of windows in the bottom right corner of the screen.
...
* Wrong window may receive focus in multi monitor setup: https://github.com/nahSystemu/HyprSpace/issues/101[#101] (Bug in Apple API)
* Wrong borderless Alacritty window may receive focus in *single monitor* setup: https://github.com/nahSystemu/HyprSpace/issues/247[#247] (Bug in Apple API)
* Performance issues: https://github.com/nahSystemu/HyprSpace/issues/333[#333]
* macOS randomly switches focus back: https://github.com/nahSystemu/HyprSpace/issues/289[#289]

When `Displays have separate Spaces` is enabled,
moving windows between monitors causes windows to move between different Spaces which is not correctly handled by the public APIs HyprSpace uses,
...
|👍 Public Apple API are more stable (which in turn affects HyprSpace stability)
...
[[on-window-detected]]
    ...
    if.during-hyprspace-startup = true
...
Please post your use cases to https://github.com/nahSystemu/HyprSpace/issues/20[the issue] if you want other commands to get supported.
...
|`if.during-hyprspace-startup`
a|
* If `true` then run the callback only during HyprSpace startup.
* If `false` then run callback only *NOT* during HyprSpace startup.
...
* `if.during-hyprspace-startup = true` is useful if you want to do the initial app arrangement only on startup.

* `if.during-hyprspace-startup = false` is useful if you want to relaunch HyprSpace, but the callback has side effects that you don’t want to run on every relaunch.
...
* You can use xref:commands.adoc#list-apps[`hyprspace list-apps`] CLI command to get IDs of running applications
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger hyprspace_workspace_change FOCUSED=$HYPRSPACE_FOCUSED_WORKSPACE'
]
----

Besides the <<exec-env-vars,`exec.env-vars`>>, the process has access to the following environment variables:

* `HYPRSPACE_FOCUSED_WORKSPACE` - the workspace user switched to
* `HYPRSPACE_PREV_WORKSPACE` - the workspace user switched from

For a more elaborate example on how to integrate with Sketchybar see
xref:./goodies.adoc#show-hyprspace-workspaces-in-sketchybar[]
...
. HyprSpace takes the assigned monitor of the workspace and makes the workspace visible on the monitor
. HyprSpace focuses the workspace
...
[NOTE]
====
The only difference between HyprSpace and i3 is switching to empty workspaces.
When you switch to an empty workspace, HyprSpace puts the workspace on an assigned monitor; i3 puts the workspace on currently active monitor.

* I find that HyprSpace model works better with <<observation,_the observation_>> listed above.
* HyprSpace model is more consistent (it works the same for empty workspaces and non-empty workspaces)
====
...
HyprSpace uses the API to gently ask windows whether they are dialogs, but HyprSpace also applies some heuristics.
...
It's fine to hardcode special handling for popular applications, HyprSpace already does it.
Please see `isDialogHeuristic` function in HyprSpace sources.
...
If you can't make HyprSpace handle some keys in your config, please make sure that you don't have keys conflict with other software that might listen to global keys (e.g. skhd, Karabiner-Elements, Raycast)
