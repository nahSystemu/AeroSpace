...
Do you have a cool automatization, HyprSpace integration, or workflow?
...
You can also use `after-startup-command` to start JankyBorders together with HyprSpace

[source,toml]
----
after-startup-command = [
    # JankyBorders has a built-in detection of already running process,
    # so it won't be run twice on HyprSpace restart
    'exec-and-forget borders active_color=0xffe1e3e4 inactive_color=0xff494d64 width=5.0'
]
----

[#raycast-extension]
== HyprSpace Raycast extension

There is a third party Raycast extension for HyprSpace. https://www.raycast.com/limonkufu/hyprspace
...
[source,bash]
----
hyprspace workspace "$(hyprspace list-workspaces --monitor mouse --visible)" && hyprspace workspace next

hyprspace workspace "$(hyprspace list-workspaces --monitor mouse --visible)" && hyprspace workspace prev
----
...
* https://github.com/acsandmann/hyprspace-swipe
* link:https://folivora.ai/[BetterTouchTool] can assign arbitrary commands to trackpad gestures.
Beware that you might need to specify full path to hyprspace executable https://community.folivora.ai/t/how-to-execute-terminal-command-to-switch-workspaces-with-hyprspace/35914
* https://github.com/MediosZ/SwipeHyprSpace
...
[#show-hyprspace-workspaces-in-sketchybar]
== Show HyprSpace workspaces in Sketchybar

You can integrate HyprSpace workspace indicators with link:https://github.com/FelixKratz/SketchyBar[Sketchybar].
Use these snippets as a starting point.

.~/.hyprspace.toml
[source,toml]
----
# Run Sketchybar together with HyprSpace
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger hyprspace_workspace_change FOCUSED_WORKSPACE=$HYPRSPACE_FOCUSED_WORKSPACE'
]
----
...
for sid in $(hyprspace list-workspaces --all); do
    sketchybar --add item space.$sid left \
        ...
        script="$CONFIG_DIR/plugins/hyprspace.sh $sid"
done
----

.~/.config/sketchybar/plugins/hyprspace.sh
[source,bash]
----
#!/usr/bin/env bash

# make sure it's executable with:
# chmod +x ~/.config/sketchybar/plugins/hyprspace.sh
...
[#show-hyprspace-workspaces-in-simple-bar]
== Show HyprSpace workspaces in simple-bar

link:https://github.com/Jean-Tinland/simple-bar[simple-bar] can be used to display HyprSpace workspaces.

In order to sync simple-bar with HyprSpace, add these lines to your HyprSpace config file:
...
Opening *a new window* of a program that can supports multiple windows (such as Safari or Terminal.app) can be accomplished with an AppleScript inlined in `hyprspace.toml` as follows:
